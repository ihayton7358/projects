...
def colourconvertor(colourstring):
    ...
    for bid in range (0,len(colours)-1):   #bid (block id) is made equal to each string in colours
...
def spaceinvader(xcorner,ycorner,zcorner,colour,background): #draws an animated space invader
    ...
    for count in range(0,10):  #deletes one set of arms and draws the other, in a loop, making the space invader move
...
while True:
    ...
    print ("ok then, I'll make a space invader that is " +colours[colour] +"\nand has a background colour of " +colours[background]) #tells the user what colour the space invader will be
    pos = mc.player.getPos()

    x = pos.x
    y = pos.y
    z = pos.z
    spaceinvader(x-5,y+10,z+6,colour,background)    #runs the space invader function with the colour and background the user chose near where the player is standing
